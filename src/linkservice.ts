...
export class LinkMappings{
    ...
    awsPropertiesDocumentationBaseUrl = 'https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-';
    ...
    public async getLink(awsTypeText: string): Promise<string>{
        if(awsTypeText && awsTypeText.includes('Type')){
            ...
            if(splitResourceType[0] === "AWS" && splitResourceType.length === 3){
                let resourceUrl = this.awsResourceDocumentationBaseUrl + splitResourceType[1].toLowerCase() + '-' + splitResourceType[2].toLowerCase();
                let propertiesUrl = this.awsPropertiesDocumentationBaseUrl + splitResourceType[1].toLowerCase() + '-' + splitResourceType[2].toLowerCase();
    ...
    formatText(unformattedAwsType: string): string{
        let formattedAwsTypeText = unformattedAwsType.split('Type:')[1].trim().replace(/['"]+/g, '');
